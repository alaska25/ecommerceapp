		...
		<section className="py-4 container bg-light">
			<div className="row justify-content-center">
				<div className="col-12">
					<h5 className="fw-bold">Cart ({totalUniqueItems}) Total Items: ({totalItems})Cart is Empty</h5>
						...
						{items.map((item, index)=>{
									return(
										<tr key={index}>
											...
											<button size="sm" className="btn btn-danger text-light ms-2 fw-bold" onClick={()=>removeItem(item.id)}>Remove Item</button>
											</td>
										</tr>
									)
								})
